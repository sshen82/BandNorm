...
```{r PCA Result, fig.width = 13, fig.height = 8}
library(ggplot2)
...
```{r Projection, fig.width = 13, fig.height = 8}
library(ggplot2)
library(viridis)
library(dplyr)
library(gridExtra)
...
p_celltype <- DimPlot(combinedAssay, reduction = "umap", label = TRUE, repel = TRUE, pt.size = 1.3, shape.by = "method", label.size = 12) +
xlab("UMAP 1") +
ylab("UMAP 2") +
